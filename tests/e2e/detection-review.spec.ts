...
test("review detection: relabel and approve persist through reload", async ({ page }) => {
  ...
  await page.locator('[data-testid="edit-label-0"]').click();
  await page.locator('#labelClassInput').fill('9');
  await page.locator('#saveLabelClassBtn').click();

  await page.locator('#reviewApproveBtn').click();
  ...
  await expect(page.locator('#label-list')).toContainText('9');
});
