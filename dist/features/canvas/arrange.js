...
export function extractVisibleRectSelection(activeObject) {
    if (!activeObject) {
        return [];
    }
...
export function toArrangedRects(rects) {
    return rects.map((rect) => ({
        rect,
        bounds: getRectBounds(rect)
    }));
}
export function sortRectGeometrically(rects, axis) {
...
export function calculateSelectionBounds(rects) {
    if (rects.length === 0) {
        return null;
    }
    const arranged = toArrangedRects(rects);
...
export function planEdgeAlignment(rects, edge) {
    if (rects.length < 2) {
        return [];
    }
...
export function planEqualEdgeGapDistribution(rects, axis) {
    if (rects.length < 3) {
        return [];
    }
