...
function inflateStoredZlib(data) {
    if (data.length < 6) {
...
function encodeMaskPixels(snapshot) {
...
function decodeMaskPixels(width, height, bytes) {
...
function createMetadata(snapshot) {
    return {
    ...
    try {
        ...
        return {
            ...
            hiddenClassIds: Array.isArray(parsed.hiddenClassIds)
                ? parsed.hiddenClassIds.filter((value) => typeof value === "string")
                : [],
...
export function encodeSegmentationMaskPng(snapshot) {
...
export function decodeSegmentationMaskPng(input) {
...
export function createSegmentationAnnotationCodec() {
    return {
