...
function toFabricPoint(point: { x: number, y: number }): fabric.Point {
    return new fabric.Point(point.x, point.y) as fabric.Point;
}
...
const colorPalette: string[] = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080', '#ffffff', '#000000', '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f'];

function getColorForClass(labelClass: string): string {
...
class AppState {
    imageFolderHandle: FileSystemDirectoryHandle | null = null;
    labelFolderHandle: FileSystemDirectoryHandle | null = null;
    imageFiles: FileSystemFileHandle[] = [];
    currentImageFile: FileSystemFileHandle | null = null;
    currentImage: fabric.Image | null = null;
    currentMode: 'draw' | 'edit' = 'draw';
    isAutoSaveEnabled: boolean = false;
    saveTimeout: number | null = null;
    currentLoadToken: number = 0;
    _clipboard: fabric.Object | null = null;
}
...
class UIManager {
    ...
    constructor(state: AppState, canvasController: CanvasController, fileSystem: FileSystem) {
        this.state = state;
    ...
    getDOMElements(): DOMElements {
        return {
            ...
            imageSearchInput: document.getElementById('imageSearchInput') as HTMLInputElement,
            saveLabelsBtn: document.getElementById('saveLabelsBtn')!,
            autoSaveToggle: document.getElementById('autoSaveToggle') as HTMLInputElement,
            drawModeBtn: document.getElementById('drawMode') as HTMLInputElement,
            editModeBtn: document.getElementById('editMode') as HTMLInputElement,
            ...
            coordXInput: document.getElementById('coordX') as HTMLInputElement,
            coordYInput: document.getElementById('coordY') as HTMLInputElement,
    ...
    updateLabelList(): void {
        ...
        const rects = this.canvasController.getObjects('rect') as CustomFabricRect[];
        ...
        rects.forEach((rect, index) => {
            ...
            li.addEventListener('click', (e: MouseEvent) => {
                if ((e.target as HTMLElement).closest('.edit-btn') || (e.target as HTMLElement).closest('.delete-btn')) return;
        ...
        const activeClassFilters = new Set<string>();
        this.elements.labelFilters.querySelectorAll('.btn[data-label-class].active').forEach(btn => {
            activeClassFilters.add((btn as HTMLElement).dataset.labelClass!);
        });
    ...
    updateLabelFilters(rects: CustomFabricRect[]): void {
    ...
    addEditDeleteListeners(rects: CustomFabricRect[]): void {
    ...
    updateImageInfo(fileName: string): void {
        this.elements.currentImageNameSpan.textContent = fileName;
    }

    setActiveImageListItem(imageFile: FileSystemFileHandle): void {
    ...
    handleDrop(e: DragEvent): void {
        ...
        if (this.dragSrcEl && this.dragSrcEl !== e.target) {
            ...
            const destIndex = parseInt((e.target as HTMLElement).closest('li')!.dataset.index!, 10);
...
class FileSystem {
    ...
    constructor(state: AppState, uiManager: UIManager, canvasController: CanvasController) {
        this.state = state;
    ...
    async listImageFiles(): Promise<void> {
        ...
        for await (const entry of this.state.imageFolderHandle.values()) {
            if (entry.kind === 'file' && /\.(jpg|jpeg|png|gif|tif|tiff)$/i.test(entry.name)) {
                this.state.imageFiles.push(entry as FileSystemFileHandle);
            }
        }
        this.uiManager.renderImageList();
    }

    async loadImageAndLabels(imageFileHandle: FileSystemFileHandle): Promise<void> {
        ...
        const setBackgroundImage = (img: fabric.Image) => {
    ...
    async saveLabels(isAuto: boolean = false): Promise<void> {
...
interface CustomFabricRect extends fabric.Rect {
    labelClass: string;
    originalYolo: { x_center: string, y_center: string, width: string, height: string } | null;
    isIssue: boolean;
}
...
class CanvasController {
    ...
    startPoint: fabric.Point | null = null;
    currentRect: fabric.Rect | null = null;
    activeLabelText: fabric.Text | null = null;

    constructor(state: AppState, uiManager: UIManager | null, fileSystem: FileSystem | null) {
        this.state = state;
        this.uiManager = uiManager;
        this.fileSystem = fileSystem;
    ...
    setBackgroundImage(img: fabric.Image): void {
    ...
    getLabelsAsYolo(): string {
        const rects = this.getObjects('rect') as CustomFabricRect[];
        const imgWidth = this.state.currentImage!.width!;
        const imgHeight = this.state.currentImage!.height!;
        let yoloString = '';
    ...
    finishDrawing(): void {
        ...
        if (this.currentRect!.width! < 5 && this.currentRect!.height! < 5) {
            ...
            (this.currentRect as CustomFabricRect).labelClass = finalLabel;
    ...
    editLabel(rect: CustomFabricRect): void {
    ...
    // Zoom and Pan
    zoom(factor: number, point?: { x: number, y: number }): void {
        const center = point ? toFabricPoint(point) : this.canvas.getCenter() as any;
    ...
    // Selection Info
    updateSelectionLabel(e: fabric.IEvent): void {
        ...
        const activeObject = e.selected[0] as CustomFabricRect;
...
class EventManager {
    ...
    bindEventListeners(): void {
        ...
        this.ui.elements.autoSaveToggle.addEventListener('change', (e: Event) => {
            this.state.isAutoSaveEnabled = (e.target as HTMLInputElement).checked;
        ...
        const markAsModified = (e: fabric.IEvent) => {
            if (!e.target) return;
            const target = e.target;
            if (target.type === 'activeSelection') {
                (target as fabric.ActiveSelection).getObjects().forEach(obj => (obj as CustomFabricRect).originalYolo = null);
            } else {
                (target as CustomFabricRect).originalYolo = null;
            }
        };
    ...
    handleMouseDown(opt: fabric.IEvent<MouseEvent>): void {
        const evt = opt.e;
        if (evt.altKey || evt.ctrlKey) {
            (this.canvas.canvas as any).isDragging = true;
            this.canvas.canvas.selection = false;
            (this.canvas.canvas as any).lastPosX = evt.clientX;
            (this.canvas.canvas as any).lastPosY = evt.clientY;
        } else {
            this.canvas.startDrawing(this.canvas.canvas.getPointer(evt));
        }
    }

    handleMouseMove(opt: fabric.IEvent<MouseEvent>): void {
        if ((this.canvas.canvas as any).isDragging) {
            const vpt = this.canvas.canvas.viewportTransform!;
            vpt[4] += opt.e.clientX - (this.canvas.canvas as any).lastPosX;
            vpt[5] += opt.e.clientY - (this.canvas.canvas as any).lastPosY;
            this.canvas.renderAll();
            (this.canvas.canvas as any).lastPosX = opt.e.clientX;
            (this.canvas.canvas as any).lastPosY = opt.e.clientY;
        } else {
    ...
    handleMouseUp(): void {
        if ((this.canvas.canvas as any).isDragging) {
            ...
            (this.canvas.canvas as any).isDragging = false;
    ...
    handleMouseWheel(opt: fabric.IEvent<WheelEvent>): void {
    ...
    copy(): void {
        ...
        activeObject.clone((cloned: fabric.Object) => { this.state._clipboard = cloned; }, ['labelClass', 'originalYolo']);
    }

    paste(): void {
        if (!this.state._clipboard) return;
        this.state._clipboard.clone((cloned: fabric.Object) => {
            this.canvas.canvas.discardActiveObject();
            const newObjects: fabric.Object[] = [];

            const pasteObject = (obj: fabric.Object) => {
                const newObj = fabric.util.object.clone(obj) as CustomFabricRect;
            ...
            if (cloned.type === 'activeSelection') {
                (cloned as fabric.ActiveSelection).forEachObject(obj => newObjects.push(pasteObject(obj)));
            } else {
    ...
    changeSelectedClasses(): void {
        ...
        const newLabel = prompt(`Enter new class for selected object(s):`, (activeSelection as CustomFabricRect).labelClass || '0');
        if (newLabel === null || newLabel.trim() === '') return;

        const finalLabel = newLabel.trim();
        const color = getColorForClass(finalLabel);

        const applyChanges = (obj: fabric.Object) => {
            const customObj = obj as CustomFabricRect;
        ...
        if (activeSelection.type === 'activeSelection') {
            (activeSelection as fabric.ActiveSelection).forEachObject(applyChanges);
        } else {
